...
@section('content_head')
    @include('master_layouts.includes.menu_content_head', [
        'title_page' => __('title_page.' . $route),
    ])
@endsection
@section('content')
    <div class="">
                            ...
                            columns: [

                                {
                                    ...
                                    template: function(row) {
                                        return '<img class="" style=";width:60px;height: 60px" src=/storage/"' +
                                            row.main_image + '" id="' + row.id + '">';
                                    },
                                    width: 100
                                },
                                ...
                                },
                                {
                                    ...
                                    template: function(row) {
                                        if (row.status === 'active') {
                                            q
                                            return '<span class="kt-badge ' + 'kt-badge--brand' +
                                                ' kt-badge--inline kt-badge--pill">' + row.status +
                                                '</span>';

                                        }

                                        if (row.status === 'pending') {
                                            return '<span class="kt-badge ' + 'kt-badge--danger' +
                                                ' kt-badge--inline kt-badge--pill">' + row.status +
                                                '</span>';
...

                                {
                                    ...
                                    autoHide: false,
                                    template: function(row) {
                                        return '<span id="create' + row.id + '">' + row.created_at.split(
                                            ' ')[0] + '</span>';
                                    }
                                },
                                @if (!(auth()->user()->status == 'suspend'))
                                    ...
                                    return '\
                                    \
                                    <a href="#" data-href="/reseller/publisher/'+row.id+'/websites"
                                        class="btn btn-sm btn-clean btn-icon btn-icon-sm btn-view openInNewTap" data-id="'+row.id +'" title="View">\
                                        <i class="fa fa-eye"></i>\
                                    </a>\
                                    \
                                    <a href="#" data-href="/reseller/publisher/'+row.id+'/profile" data-id="'+row.id+'"
                                        class="btn btn-sm btn-clean btn-icon btn-icon-sm btn-edit openInNewTap" \ title="Edit">\
                                        <i class="fa fa-edit"></i>\
                                    </a>\
                                    ';
                                    }
                                    
                                    if(row.status =='disable' ||row.status =='pending' ){
                                    return '\
                                    \
                                    <a data-href="/reseller/publisher/'+row.id+'/websites"
                                        class="btn btn-sm btn-clean btn-icon btn-icon-sm btn-view openInNewTap" data-id="'+row.id +'" title="View">\
                                        <i class="fa fa-eye"></i>\
                                    </a>\
                                    \
                                    <a data-href="/reseller/publisher/'+row.id+'/profile" data-id="'+row.id+'"
                                        class="btn btn-sm btn-clean btn-icon btn-icon-sm btn-edit openInNewTap" \ title="Edit">\
                                        <i class="fa fa-edit"></i>\
                                    </a>\
                                    ';
