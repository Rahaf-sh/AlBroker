...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.negotiation.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.negotiations.update", [$negotiation->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('operator') ? 'is-invalid' : '' }}" name="operator_id" id="operator_id" required>
                    @foreach($operators as $id => $entry)
                        <option value="{{ $id }}" {{ (old('operator_id') ? old('operator_id') : $negotiation->operator->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('charterer') ? 'is-invalid' : '' }}" name="charterer_id" id="charterer_id">
                    @foreach($charterers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('charterer_id') ? old('charterer_id') : $negotiation->charterer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cargo') ? 'is-invalid' : '' }}" name="cargo_id" id="cargo_id">
                    @foreach($cargos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('cargo_id') ? old('cargo_id') : $negotiation->cargo->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('cargo'))
