...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.doneNegotiation.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.done-negotiations.update", [$doneNegotiation->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('negotiation') ? 'is-invalid' : '' }}" name="negotiation_id" id="negotiation_id" required>
                    @foreach($negotiations as $id => $entry)
                        <option value="{{ $id }}" {{ (old('negotiation_id') ? old('negotiation_id') : $doneNegotiation->negotiation->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('operator') ? 'is-invalid' : '' }}" name="operator_id" id="operator_id">
                    @foreach($operators as $id => $entry)
                        <option value="{{ $id }}" {{ (old('operator_id') ? old('operator_id') : $doneNegotiation->operator->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('charterer') ? 'is-invalid' : '' }}" name="charterer_id" id="charterer_id">
                    @foreach($charterers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('charterer_id') ? old('charterer_id') : $doneNegotiation->charterer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
